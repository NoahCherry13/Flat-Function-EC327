...
class RateMyProfApi:
    ...
    def GetID(self, ln):
        ...
        for profID in uci.professors:
            if uci.professors[profID].last_name == ln:
        ...
        if (count >1):
            ...
            for profID in uci.professors:
                if uci.professors[profID].last_name == ln:
                    print(uci.professors[profID].department)

            department = input()
            for profID in uci.professors:
                if uci.professors[profID].last_name == ln and uci.professors[profID].department == department:
                    uci.professorID = profID
                    return uci.professorID
        else:
            return uci.professorID

    def GetAverageRating(self,name):
            return uci.professors[uci.GetID(name)].overall_rating
...
if __name__ == '__main__':
    ...
    #print(uci.professors[1553112].overall_rating)
    professor_names = ["Lin", "Goh"]
    for names in professor_names:
        print(uci.GetAverageRating(names))
